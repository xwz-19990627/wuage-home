...
| 公网网关 | 0.0.0.0:8443 | 登录认证 + 反代（/ 到 3081，/ledger/ 到 8000） | systemd: dsh-public.service |
| wuage 面板 | 127.0.0.1:8000 | 家庭账本面板（读写 data/ledger.db） | systemd: wuage-panel.service |
...
- 行为：未登录 → 跳 /login；已登录 → /ledger/* 反代到 http://127.0.0.1:8000（去掉前缀）。
...
| 场景 | 地址 | 说明 |
| --- | --- | --- |
| 服务器本机 | http://127.0.0.1:8000 | 无需登录 |
