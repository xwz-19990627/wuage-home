...
- 格式 `v0.x.y`：
  - `y`（PATCH）= **默认迭代单位**。每次“完整小改动”（实现 → 测试 → CHANGELOG → 发布）
    就升一位：v0.1.0 → v0.1.1 → v0.1.2，可以一天多个，也可以几天一个。
  - `x`（MINOR）= **新模块**正式引入时升（如 v0.1.x 记账 → v0.2.0 财务健康系列正式引入）。
  - `MAJOR` 留到公开稳定（对外使用）时升 1。
- 规则：**不屯改动**——做一点、记一点、发一点；同一主题的半成品改动可合并成一个小版本。
